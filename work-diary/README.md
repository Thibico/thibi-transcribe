...
| [2026-08-12](./2026-08-12.md) | The presigned-URL host gap, and the identity matching it exposed as never having run through any path a user has — plus the plan's own demo, which passed while proving nothing · `thibi transcribe --job <id>` and the guard that makes it safe · the sidecar contract test, run against the real model rather than the canned pipeline the plan asked for |
