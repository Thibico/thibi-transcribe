...
Dependency direction is one-way: `core ← languages ← db ← engine ← runtime ← {web, cli, worker}`.
`runtime` is not an app: it is the composition root the three apps share, it is where the
environment is read, and no package may import it (amendment 93).
...
| 83 | **The plan's own entity regex fails on the one edit a cleanup prompt exists to make, and the gate printed "pass" on a run that measured nothing.** Both found by the first live LLM eval, 2026-08-14, and neither by the 41 tests written before it. §5.10 specifies `entity_drift` over `/\p{Lu}{2,}\|\d[\d.,:/٫٬]*\|…/gu`; the digit alternative is greedy over its separator class, so a compliant pass adding a full stop turned `…kasa a 1755` into `…kasa a 1755.`, `1755` left the multiset and `1755.` joined it — **entity_drift 2.0 against a 0.02 gate**, i.e. every arm in every language would have failed the gate for adding punctuation. Separators now require a digit after them, which keeps `1,000.50`, `12:30` and `1/2` whole. **The Latin-token branch had the identical defect and the same run found it**: Burmese `ring ၏ ceo` came back as `ring၊ ၏ ceo၊` — a clause mark inserted and nothing else changed — and the `(?=$|\s)` lookahead stopped matching, so both tokens "left the text" and the restraint arm scored **entity_drift 0.25** for words it had not altered. Both boundaries are now the absence of another character of the same class rather than the presence of whitespace, because punctuation attached to a token is precisely what a typesetting pass produces. The second finding is worse: a six-language n=30 run lost five languages to one Groq 429 apiece and one to a `json_validate_failed` 400, because a call error propagated out of the language loop — and the gate then printed **"pass — every arm is at or below its control"** and exited 0, having skipped every language it had nothing to compare. Three fixes: a failed call costs its segment and not its language; a language the run asked for and did not measure is a gate **failure** (`not_measured`), not a skip; and a run where nothing was measured exits non-zero with or without `--gate`. The general form is the one this project keeps paying for — **a check that cannot distinguish "clean" from "did not run" is not a check** — and the specific lesson is that the harness needed the retry ladder the engine already had: `withRetry` with `RETRY_POLICIES['editorial.pass']`, honouring the wait Groq states in its 429 body. | [05](./phase-05-eval-harness.md), [06](./phase-06-llm-passes.md) |
| 84 | **Two registry fields the prompt design assumes do not exist, and one of them cannot be derived from Unicode.** Phase 6 §6.4's mapping table reads `script.hasCase` and `text.punctuation.clause`; the registry has neither. Both are now script-level hand judgements in `gen-scripts.ts`. `hasCase` is orthographic rather than a character property — Georgian Mtavruli capitals carry `Lu`, so a derivation from categories would have the cleanup prompt instructing a model to capitalise Georgian sentences, which standard Mkhedruli does not do — and it is `true` for Latn, Cyrl, Grek and Armn only. `clausePunct` exists because `text.punctuation.sentenceEnders` is one flat list that several languages fill with both kinds of mark: **Burmese lists `၊` beside `။`, Amharic `፣` beside `።`**, so a prompt offering that list as "sentence-ending punctuation" tells a model the Burmese comma ends sentences. `promptVars` subtracts one set from the other, in one place rather than in 116 language rows. **Empty means not recorded, not "this script has none"** — an empty set omits the permission line entirely, which is the safe direction, since the prompt is a list of what may be changed and an unlisted mark is one the model may not insert. Thai and Lao are empty because they genuinely do not punctuate clauses; Thaana is empty because nobody here has checked it. | [05](./phase-05-eval-harness.md), [06](./phase-06-llm-passes.md) |
| 85 | **The research finding reproduces, and the restraint prompt reverses it — in five of six languages, not six.** First cleanup measurement, 2026-08-14, `groq/openai/gpt-oss-20b`, n=10, `cer_punct` against a do-nothing control: `my-MM` 0.016 → current 0.021 → restraint 0.027; `yo-NG` 0.032 → **0.096** → 0.018; `ps-AF` 0.017 → 0.030 → 0.014; `so-SO` 0.043 → 0.044 → 0.030; `ha-NG` 0.036 → 0.028 → 0.022; `xh-ZA` 0.042 → 0.051 → 0.029. The shipped prompt is worse than doing nothing in five of six — **Hausa is the exception**, which the research's "every language tested" does not admit — and the replacement beats the control in five of six. `content_delta` is the sharper instrument, being a contract rather than a score: the shipped prompt rewrote **9.5% of Yoruba's characters**, 2.4% of Somali's, 2.0% of Xhosa's, and the restraint prompt sits at or near zero everywhere. **Burmese fails exactly where phase-06 risk 1 predicted**, at 0.027 against a 0.016 control and further above it than the research's own 0.019 — and the run says *how*: it inserts `၊` at every phrase boundary where the reference has a space, with `content_delta` at **0.0000**. The contract held perfectly and the punctuation it was licensed to choose was wrong, which is a prompt problem with a clear shape and Phase 6's to close. Not relaxed. Two caveats on the numbers: n=10 rather than 30, because Groq's on-demand tier caps this model at 8000 tokens per minute; and **an arm is scored over the segments that came back while the control is scored over all of them**, since Groq 400s cost `yo-NG current` three of its ten and those three are not a random three. Every arm of a language should be scored over the intersection of what all arms returned — open, and it changes numbers. | [05](./phase-05-eval-harness.md), [06](./phase-06-llm-passes.md) |
| 86 | **`partial` was unreachable, twice over, in the design that exists to deliver it.** Phase 9 §9 is entirely about a three-hour transcript with one bad 55-second chunk still being worth having; its reconciler could not produce that outcome. `asr.chunk` is not `optional`, so one dead shard made `hardFailed` true and §3's terminal branch chose `failed` before it ever reached the `partial` test — and independently, §3's dependency-poisoning rule marked `normalize.text`, which depends on `['asr.chunk','*']`, `failed` the moment any shard died, so the seven survivors would never have been assembled either. Both close with one rule rather than two patches: a `dead` step of a **casualty kind** satisfies its dependents and does not fail the run, *provided a sibling shard of that kind succeeded*. The proviso is what keeps it honest — if none did, nothing was transcribed and the run has genuinely failed, which is why the all-chunks-dead case still lands on `failed`. What makes proceeding over a casualty safe at all is the placeholder segment: the timeline stays contiguous, so subtitle reflow, export, the editor's virtualiser and speaker reconciliation each see the shape they already expect rather than a hole needing a special case. Confirmed by emptying `CASUALTY_KINDS`, which fails the partial test and leaves the all-dead one passing. | [09](./phase-09-queue-and-worker.md) |
| 87 | **Three smaller defects in the same phase, each of which would have shipped silently.** (a) §6's queue table, its `SUBSCRIPTIONS` map and its default `WORKER_QUEUES` all omit the `worker` queue that §2's step-kind table routes `normalize.text` and `reconcile.speakers` to. Every run would plan two steps onto a queue no container subscribes to; they sit `ready` forever while the queue reports depth 0 — the exact misconfiguration §12 tells an operator to watch for, authored into the plan. A test now asserts some container subscribes to every queue that exists. (b) §4's materialise SQL uses `array_agg(dep.id)` with no `ORDER BY`, so replanning an unchanged run may rewrite `depends_on` with the same uuids in a different order, `is distinct from` stops suppressing the update, and §-Tests' own "byte-identical across both calls" assertion fails **intermittently** — the worst way, because it passes on most runs. Removing the ordering fails it 3 of 3. The idempotence tests assert on `xmin` rather than on column values, since `xmin` advances on any UPDATE including one writing what was already there. (c) §-Tests asserts twenty parallel `reconcile` calls produce "exactly one send per step". They do not and must not: §3 deliberately re-rings anything already `ready` to cover the window between COMMIT and `sendStep`. The property that actually prevents double execution is that all those sends carry the **same singleton key**, and that is what is asserted instead. | [09](./phase-09-queue-and-worker.md) |
| 88 | **Two retry tables for the same step kinds had already drifted, and neither number was measured.** `packages/engine/src/retry.ts` capped `asr.chunk` backoff at 30 s and `diarize` at 60 s; phase 9 §5's table says 120 s and 300 s. The overview specifies attempts and base delay per kind and never specified a cap, so each file invented one and nothing compared them. There is now one table — `queue/retry.ts`'s `POLICY` — and the in-process one derives from it under a test, which makes "which is right?" unaskable rather than merely answered. Widening `isRetryable` at the same time closed a real gap rather than a stylistic one: **Node and undici report connection failures on `err.code`, not in the message**, so the message-only regex classified every `UND_ERR_CONNECT_TIMEOUT` and `UND_ERR_HEADERS_TIMEOUT` — the most common transient failure this project sees — as permanent. Phase 9's `CancelledError` is the existing `AbortedError` rather than a second class. | [09](./phase-09-queue-and-worker.md) |
| 89 | **`step_state` is a real Postgres enum, against this schema's own convention, and `cost_usd` follows the convention against the plan.** Every other state column here is `text` with a Drizzle `enum` option, which is a TypeScript constraint only — the column is plain text and the database accepts anything. `run_steps` is the one table most of whose writes are hand-written SQL (the planner's `jsonb_to_recordset`, the recovery sweep's `UPDATE … CASE`, the reconciler's casts), none of which sees a TypeScript type, so a typo'd state would be stored and then match no predicate: a step neither live nor terminal, invisible to both the reconciler and `/admin/queue`. Adding a state is now a migration, which is the right price for a column whose job is to be exhaustively matched. `kind` stays `text` for the opposite reason — kinds are dispatched through a registry keyed by string. Conversely `cost_usd` is `double precision` rather than the plan's `numeric(12,6)`, matching `runs.cost_usd` and `usage_records.usd`: a step's cost sums into a run's, and a sum mixing the two returns `double precision` anyway, so the stricter type buys nothing and costs consistency. | [09](./phase-09-queue-and-worker.md) |
| 90 | **pg-boss's default queue policy makes the whole re-ring design silently wrong, and the library is two majors past what the plan assumes.** Phase 9 §3 re-rings every `ready` step on every 30-second pass and says "the singleton key makes the re-send free". Under pg-boss's default `standard` policy that is false: a `singletonKey` there is inert unless paired with `singletonSeconds`, because dedup on `standard` is a throttling feature. A run whose reconciler ticked forty times would have queued forty jobs for one step, so the queue grows without bound on any run longer than half a minute. Every queue is now declared with policy **`short`** — at most one job *queued* per key, enforced by a unique index predicated on the policy — and a test asserts the policy rather than trusting a default. `short` deliberately rather than `stately`/`exclusive`, which also exclude the *active* job: a re-ring while a step runs is already harmless, because `runStep` claims with `attempt = $n` and finds `running`, and the stricter policies would block a legitimate retry behind a job still finishing. Three further v10→v12 differences, all found by compiling and running: `PgBoss` is a named export where it was a default; `StopOptions.wait` is gone because `stop()` now blocks until the drain completes; and `stop()` returns early on a second call, so the plan's `stop({close:false})`-then-close sequence would silently leak every connection. pg-boss owns its own pool — sharing `ctx.db`'s would put its one-second poll loop in contention with handler queries, taking out both at once under load. Node floor raised to 22.12.0 to match what pg-boss declares, since `.npmrc` sets `engine-strict=true`. | [09](./phase-09-queue-and-worker.md) |
| 91 | **The reconciler claimed runs it had not planned, and only starting the worker showed it.** The 30-second tick reconciles every live run; the CLI drives the same stages in one process and never plans a DAG, so every CLI-created run has zero `run_steps`. `reconcile` found nothing outstanding, fell through to its `pending` → `running` transition, and marked a run no worker would ever touch as running at 0% — permanently. The dev database had one sitting there from before the phase existed, and the first boot log read `reconciled=1`. `reconcile` now returns early on a stepless run: an unplanned run is not the queue's business. **Every test in the suite plans a DAG first, which is exactly why none of them could have produced this** — the general form is the one this project keeps re-learning, that starting the thing finds what testing the thing cannot. | [09](./phase-09-queue-and-worker.md) |
| 92 | **`awaiting_external` steps are nudged, never reset, and that one statement is the difference between a restart costing a poll cycle and costing $19.** Phase 9 §8(b), built and covered. A step in `awaiting_external` is running on someone else's computer; treating it like a stale `running` step re-submits work already in flight and pays for it twice, silently, for two hours, showing up only on the bill. The sweep drops `poll_after` to now and touches nothing else — its test asserts `state`, `external_ref` and `attempt` are all unchanged. The mirror-image rule on the same path: a stale *lease* is reclaimed with `attempt + 1`, because a worker that vanished mid-chunk usually vanished for a reason that repeats, and a step reclaimable without limit loops forever against an OOM. | [09](./phase-09-queue-and-worker.md) |
| 93 | **Two apps needing the same `EngineContext` meant one composition root, not two copies.** Phase 9's worker assembles the same store, ffmpeg port, settings, staging, temp directory and logger from the same environment as the CLI. `apps/cli/src/context.ts` moved to `@thibi/runtime` rather than being copied — copying was the cheap option and it is the mistake this project had just paid for twice in one day (two retry tables that had drifted on caps neither of them measured; two queue tables that disagreed about a queue's existence). A second builder would have drifted on exactly the details that are invisible until they matter: the SigV4 signing-client split that took a spike to diagnose, the `THIBI_TMP_DIR` validation that exists so a bad mount fails before money is spent. **It lives under `apps/` rather than `packages/` deliberately**: an ESLint rule bans `process.env` under every package's `src`, so putting an env reader there would have meant carving an exemption into a rule the repo calls architecture rather than style. `runtime` is listed alongside the apps in the layer rule, so no package may import it either. Three things are genuinely per-app and became parameters: the Postgres `application_name`, the Google region/model defaults, and the `EventSink` — the CLI prints progress to stderr, the worker inserts `run_events` and NOTIFYs. | [09](./phase-09-queue-and-worker.md) |
| 94 | **A wildcard dependency over a kind with no shards yet is vacuously satisfied, and it made the first real run transcribe nothing and report success.** `startRun` planned with `chunkCount = 0`, which emitted zero `asr.chunk` shards *and* every step downstream of them — including `normalize.text`, whose `['asr.chunk','*']` dependency then resolved to an empty array. A step with no dependencies is a root, so the reconciler promoted it on the first tick, a worker assembled a transcript out of nothing, and the run reached `done` at progress 1 with **zero segments** while twenty `asr.chunk` steps were still queued. Sixteen minutes of Burmese, $0.27, nothing kept. The fix is to distinguish two statements the plan had conflated: `planRun` takes `number | null`, and `null` — "I do not know yet" — stops the plan after `plan.chunks`, because everything past that point depends directly or transitively on a wildcard with nothing to resolve to. `batch` is exempt: its ASR steps are not sharded, so the count it does not know is not one it needs. **The reconciler cannot make this distinction itself** — by the time it reads `depends_on` the array is `uuid[]` and a collapsed wildcard is indistinguishable from a genuine root — so the planner is the last place that still knows. The existing test asserted only that the first pass emitted no `asr.chunk` shards, which was true throughout and told nobody anything. | [09](./phase-09-queue-and-worker.md) |
| 95 | **Three more things the first live run found, none of which a test would have.** (a) The second planning pass inserts the `asr.chunk` shards *between* `plan.chunks` and `normalize.text`, so every ordinal after them moves — and `ON CONFLICT DO NOTHING` left the already-inserted rows on the first pass's numbering. `normalize.text` ended up at ordinal 3 beside `asr.chunk` shard 0. Ordinal order is a topological order and `reconcile` walks it, so this is the invariant rather than a display detail; the insert is now `DO UPDATE SET ordinal` guarded by `WHERE ordinal IS DISTINCT FROM`, which repairs the numbering while leaving planning a genuine no-op. (b) **`WORKER_HEALTH_PORT` defaulted to 8081, which is the diarization sidecar's port** in this repo's own `infra/compose.dev.yml` and in `.env` as `SIDECAR_URL` — so the worker refused to start with `EADDRINUSE` on every box that can diarize. Now 8090. (c) **A JSON `null` is not a SQL NULL**: `createRun` wrote `JSON.stringify(probeRaw ?? null)` into a `jsonb` column, storing the document `null`, so `probe_raw is not null` was true for every asset ever inserted. `media.probe` asks exactly that to decide whether it can skip downloading a 2 GB file, so it would have skipped always, on assets nothing had ever probed. | [09](./phase-09-queue-and-worker.md) |
| 96 | **The seam merge decides where persistence lives, and phase 9 put it in the wrong step.** §9 and the porting table both place `lib/queue.ts:112-113`'s comment — *"Insert per chunk rather than at the end, so a long file shows partial results as it goes and a late failure doesn't discard earlier work"* — in `handlers/asr-chunk.ts`. The second half survives; the first cannot, and the reason is a feature the old app did not have. Chunks start 1200 ms early so the LCS merge can drop the words said twice, so a chunk's **leading** words are not final until its predecessor's are known. Rows written per chunk would have to be deleted and renumbered at every seam, on the one table whose defining rule is that the machine's output is never overwritten. So `asr.chunk` writes its parsed result to object storage under `runs/{id}/results/{idx}.json` and `normalize.text` writes the segments once, in order — which is what the overview's own pipeline diagram (`ASR → reconcile → normalize-text → persist`) said all along. Nothing is lost and something is gained: the artifact's existence is *also* the re-billing guard, committed before the step row, so a worker killed between "the provider answered" and "the step was marked done" does not pay twice. **Measured**: `kill -9` with one chunk done and five running; on restart two of the five logged `chunk already transcribed; not re-sending`. Placeholder segments for dead chunks move with it, from the dead step's handler to `normalize.text`, which is the only thing that knows the whole timeline `segments.idx` is assigned across. | [09](./phase-09-queue-and-worker.md) |
| 97 | **`awaiting_external` was not a state a step could leave, and every piece of machinery around the gap was already correct.** Phase 9 §7 has `asr.batch.submit` return `awaiting_external` and a separate `asr.poll` step do the polling — but `reconcile` sent only `pending` and `ready` steps, so a `poll_after` written by a handler was read by nothing. `runStep` has always claimed `state in ('ready','awaiting_external')`, `run_steps` has always carried a partial index on `poll_after where state = 'awaiting_external'`, and `StepResult` has always had the variant. All of it was written for a caller that did not exist, through three sittings in which the handoff note said the state had "never been exercised" — which was too generous, because it was not reachable. `reconcile` now re-rings `awaiting_external` steps with `poll_after` as the send's `startAfter`, so the delay is held by pg-boss rather than by a sleeping worker; predicated on `poll_after` being set, because a re-ring with no `startAfter` would poll instantly and spin. **The plan's own §7 also cannot work as written**: `asr.poll` depends on the submit step and only `done` or `skipped` satisfies a dependency, so a submit that parked in `awaiting_external` would poll nothing forever while `asr.poll` sat `pending` behind it. The wait moves to `asr.poll`, which is where §6's queue routing already assumed it was — that kind has its own queue precisely so a sub-second poll never queues behind a forty-minute diarize. The submit's guard against a double bill becomes the persisted `BatchOp` rather than `step.externalRef`, which a `done` result cannot set: `persistOperation` commits before the step row, so it is the durable evidence the money is committed. Same shape as amendment 96's artifact guard, and for the same reason. | [09](./phase-09-queue-and-worker.md) |
| 98 | **A recovery statement that is right on boot can be wrong on a timer, and this one was on both.** §8(b)'s nudge is `poll_after = least(poll_after, now())`, which drags a *future* poll forward. On boot that is amendment 92's whole point: a restart during a two-hour `batchRecognize` costs nothing instead of a poll cycle. On the 60-second tick it silently flattens the backoff — every scheduled poll pulled back to now once a minute, so a capped 30 s → 300 s schedule becomes a flat 60 s and a two-hour operation makes 120 requests against a quota the sync path also needs. §8 says "runs on worker boot and every 60 s thereafter" over all three statements, which is right for the lease sweep and the reconcile and wrong for this one. `recoverTick` now takes it as an option and only the boot call passes it. **Invisible until something used the state**: with no handler ever returning `awaiting_external`, the statement matched zero rows on every tick for the whole phase. | [09](./phase-09-queue-and-worker.md) |
| 99 | **The third long-async pair, and the plan's `idempotency_key: step.id` is the wrong key.** `diarize` / `diarize.poll` / `reconcile.speakers` are built, with the same structural correction amendment 97 made to the batch pair: the submit ends `done` because the poll step depends on it. §7 specifies `idempotency_key: step.id` on the submit; the key Phase 3 actually shipped is `diarizeStepKey(runId)` — `"<runId>:diarize"` — and the difference is load-bearing. The key must be **reconstructible by a process that never saw the submit response**, which is what makes "a re-delivered doorbell finds the existing task" true at all. `step.id` is not: re-planning a run mints new step rows, so a resubmit under a new key starts a second job on the only GPU. Two guards, both kept — the persisted `DiarizeHandle` saves the round trip, and the sidecar's `uuid5` of the run-derived key covers the window where the handle never reached Postgres. Three deliberate differences from the batch pair: `diarize.poll` fetches as well as polls (one call to a service on our own network, against a bucket listing and an unbounded parse billed per operation); its interval is flat at 15 s rather than a capped backoff (the sidecar is ours and a status call costs nothing, so a bar that moves is worth more than politeness); and a 429 becomes `no_slot` rather than an error, because the sidecar holds one slot, a busy one is scheduling, and `diarize` has only two attempts to spend on real faults. `reconcile.speakers` is the join and the only step depending on both branches of the DAG, which is why the turns go to object storage in between rather than into `speaker_turns` — those rows are written by `persistDiarization` in the transaction that also attributes segments and words, and a turn set without its attribution is a half-finished diarization the editor renders as speakers who said nothing. | [09](./phase-09-queue-and-worker.md) |
| 100 | **A source that honestly reports zero progress scored worse than one that reported nothing, and only watching a real diarization showed it.** `stepFraction` credited a started step 0.1 "visible, honest, and never inflated upward" — but only when `output.progress` was absent. The sidecar reports `progress: 0` for the first minutes of a pyannote run, because pyannote does not stream progress and zero is how it says so, and `clamp01(0)` is 0. So the step contributed nothing to the run's bar for as long as the source was being candid, where a silent source would have contributed a tenth. Now a floor rather than a default: `max(0.1, clamp01(p))`, with any genuine report above it still winning, so nothing is guessed upward. Measured on a two-minute clip that took about fourteen minutes to diarize — the first four polls reported 0 and the fifth onward reported real fractions, so the window where this mattered was exactly the window where a user has least other evidence the run is alive. The same run also showed `diarize.poll` writing no log line between submit and completion, which on a three-hour diarization leaves an operator unable to distinguish working from wedged; it now logs per poll as `asr.poll` does. | [09](./phase-09-queue-and-worker.md) |
| 101 | **§6's advisory-lock slots cannot bound `diarize`, and §7 is the reason — the two sections contradict each other.** `withGlobalSlot` is a session-scoped `pg_try_advisory_lock` held for the duration of the function it wraps, which bounds a step that occupies a worker while the constrained resource is busy. §7's rule is that a step waiting on someone else's computer holds no worker slot, so `diarize` submits and returns in milliseconds and a lock taken around the submit releases while the GPU runs for the next three hours. There is no way to hold a session lock across `diarize` and `diarize.poll`: different steps, usually different processes, and the worker holding it may die. **§7 wins**, because holding a worker for three hours in order to hold a lock is precisely the failure this phase exists to remove. What actually bounds diarization is the sidecar — it owns the one slot, knows whether it is taken, and answers 429, which maps to the `no_slot` result §6 already specifies and reaches it without a second mechanism to disagree with the first. **The authority on a resource's capacity should be the thing that has it.** `GPU_SLOTS` is parsed and unused and should be deleted rather than left as a knob that does nothing. `LOCAL_ASR_SLOTS` is not in the same position and the lock is still right there: `asr.chunk` on `asr.local` is one synchronous step holding its worker for the whole transcription, so a session lock spans exactly the work it protects. Still unbuilt. | [09](./phase-09-queue-and-worker.md) |
| 102 | **A guarantee does not belong behind a subsystem whose failure mode is silence.** §10 propagates a cancel to a running handler over a `LISTEN run_cancel` channel every worker subscribes to. Built the other way round: the notify is sent, nothing subscribes to it yet, and the mechanism that actually reaches a running handler is the heartbeat's existing UPDATE with one extra join — `returning (r.cancel_requested_at is not null)`. A `LISTEN` needs a dedicated non-pooled client held for the life of the process, and when it drops nothing throws; cancellation would quietly stop working until somebody noticed a run that would not die. This phase already has an open investigation into exactly that shape — a poll chain that went silent for two hours with nothing logged — so putting a guarantee behind a second subsystem with the same failure mode would have been repeating the mistake while still reading the report on it. The heartbeat is self-healing by construction: the state is in the row rather than in a message that was or was not delivered, so a missed check is retried 15 seconds later, forever. The cost is up to 15 s of latency, and **a cancel that arrives late is a delay where a cancel that never arrives is a bug**. The listener stays worth building as an accelerator *over* the guarantee. Also corrected: `requestCancel` does not reconcile — it is called from places with no doorbell, so the caller reconciles when it can and the tick does it otherwise — and `cancel_requested_by` is `text` rather than a `users` reference, because the CLI has no user to name and a run's audit trail must outlive the account that triggered it. | [09](./phase-09-queue-and-worker.md) |
| 103 | **The same 429 that scheduling should absorb was quietly spending `asr.chunk`'s retry budget.** `SidecarBusyError` extends `RateLimitedError`, so `isRetryable` said yes and `onStepError` burned one of the step's five attempts on every refusal. The sidecar holds **one** slot for faster-whisper and pyannote alike — its own comment names "a diarization of this same file" as the likely holder — so a `--provider faster-whisper --diarize` run contends with itself twenty shards at a time, and after five refusals apiece the shards start landing `dead`. That is precisely the "a busy hour marks half the diarize steps dead" failure §6 introduces `no_slot` to prevent, reached from the other direction and missed because the two paths were built three sittings apart. `asr.chunk` now maps it to `no_slot` with the provider's own `Retry-After`, exactly as `diarize` maps `DiarizerBusyError`, and hands the `run_chunks` row back as it found it so a requeue does not read as an attempt there either. **`GPU_SLOTS` and `LOCAL_ASR_SLOTS` are deleted** rather than implemented: with both callers deferring to the sidecar's own accounting, §6's advisory-lock layer has no workload left to bound, and a knob that is parsed and ignored is worse than no knob because it reads as a control. `MAX_BUCKET_WAIT_MS` stays — `rate-bucket.ts` is genuinely unbuilt rather than unnecessary. | [09](./phase-09-queue-and-worker.md) |
| 104 | **One test was quietly consuming the machine the rest of the suite needed, and raising its timeout made it worse.** `pyannote.contract.test.ts` performs a real diarization — ~40 s of CPU in Docker's Linux VM for an 11-second clip — concurrently with eighty other files. The handoff has carried "on a loaded machine it starves the Postgres suites into hook timeouts" for several sittings as an occasional annoyance; nineteen new database-backed tests over 2026-08-16/17 tipped it over, and full runs began failing four tests at a time in a *different* set of suites each run (`constraints`, `persist`, `reconcile`, and the CPU-bound `hungarian`). **The first fix made it worse and that is the diagnostic**: lengthening its internal deadline from 150 s to 420 s let it consume more of the machine and the failures spread. It was never too short — it was too expensive. Vitest has no per-file isolation, so it is out of the default pass behind `pnpm test:contract`, gated on `RUN_CONTRACT` rather than split into its own project (a project is still collected by a bare `vitest run`). **The cost is recorded rather than glossed**: the one test that checks our hard-coded wire shapes against what `schemas.py` emits no longer runs by default, so the two halves can drift in silence, and it belongs in the definition of done for any change to the sidecar or `pyannote.ts`. Same shape as the sidecar's Python suite, which is already outside `pnpm test` for the same reason. Corollary, from three flaky tests of my own in the same sitting: **a test that asserts a wall-clock window is measuring the machine.** Assert the arithmetic against the state the row actually holds. | [09](./phase-09-queue-and-worker.md) |
