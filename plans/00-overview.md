...
| 59 | **Two failures that only appear when a human runs the CLI, both of which a green suite is structurally unable to catch.** Found 2026-08-12 while running the pipeline against real audio. **(a) `thibi diarize run` exits 0 when `SIDECAR_URL` is unset.** The remediation message is correct and deliberate — amendment 48 leans on it to argue that "this box does no diarization" is a supported configuration — but a zero exit code makes *"no diarization happened"* indistinguishable from *"diarization succeeded"* to any caller that is not a human reading stderr. Phase 9's queue is exactly such a caller. **(b) `THIBI_TMP_DIR` pointing at a directory that does not exist crashes with a raw `ENOENT` stack trace** from `mkdtemp`, in a CLI whose entire error taxonomy exists so a journalist never sees one. Neither is reachable from the test suite: every suite sets a temp dir that exists and a sidecar that answers, because a suite that did otherwise would be testing its own fixtures. The pattern is the one this project keeps rediscovering — **run it, build it, start it** — and it is now the third sitting in a row where the defects came from doing that rather than from re-reading code. | [03](./phase-03-diarization.md), [09](./phase-09-queue-and-worker.md) |
