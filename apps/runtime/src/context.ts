 ...
 *
 * It lives under `apps/` and not `packages/` because that distinction is the architecture,
 * not an accident of layout — an ESLint rule bans `process.env` under every package's `src`,
 * and moving an env reader in there would have meant carving an exemption into a rule the
 * repo calls architecture rather than style. `@thibi/runtime` is not an app; it is what the
 * three apps compose *with*. Packages may not import it, same as any app.
 *
 * It was `apps/cli/src/context.ts` until Phase 9 needed a worker. Copying it would have left
 * two builders of the same object, differing in whichever detail was fixed in one and not the
 * other — the retry-table mistake this project had just finished paying for.
 *
 ...
 */
export function createRuntimeLogger(level: Level = 'info', bindings: Record<string, unknown> = {}): Logger {
  ...
  return {
    child: (extra) => createRuntimeLogger(level, { ...bindings, ...extra }),
...
/** Progress to stderr; the default when a caller injects nothing. The worker injects. */
export function createLoggingEvents(logger: Logger): EventSink {
  return {
    emit(event) {
...
export interface BuildContextOptions {
  ...
  /**
   * `application_name` on the Postgres connection, e.g. `thibi-cli` or `thibi-worker`.
   *
   * Worth passing correctly: it is what `pg_stat_activity` shows, and "which of these
   * connections is the worker" is the first question asked of a database that is misbehaving.
   */
  appName: string;
   ...
   *
   * The CLI pretty-prints to stderr; the worker inserts `run_events` and NOTIFYs. It is the
   * one part of the context that genuinely differs between the two, which is why it is
   * injected rather than chosen here from a flag.
   */
  events?: (logger: Logger, db: Db | null) => EventSink;
}
...
export async function buildContext(options: BuildContextOptions): Promise<RuntimeContext> {
  ...
  if (!noDb) {
    ...
    db = createDb({ url: env.DATABASE_URL, max: 6, applicationName: options.appName });
  }
  ...
  const ctx: EngineContext = {
    ...
    clock: systemClock(),
    logger,
    events: (options.events ?? createLoggingEvents)(logger, db),
    languages,
